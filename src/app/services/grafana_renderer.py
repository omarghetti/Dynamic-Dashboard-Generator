...
def load_grafana_templates(dashboards, dashboard_style, datasource):
  rendered_dashboards = []
  dashboard_url = ""
  for item in dashboards:
    dash_template = load_template(grafana_dash_path, 'grafana_dashboard.json')
    if dash_template is not None:
      rendered_dashboard = render_grafana_templates(dash_template, item, dashboard_url, datasource)
...
def render_grafana_templates(dash_to_render, concrete_dash, dashboard_url, datasource):
  rendered_dashboard = dash_to_render.render(panels=concrete_dash.panels, dashboard=concrete_dash, links=concrete_dash.links, url=dashboard_url, datasource=datasource)
